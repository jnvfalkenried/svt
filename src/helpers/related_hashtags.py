...
def merge_frequent_itemsets(old_rules_df, new_txn_df): 
    ...
    old_copy_df["new_support"] = old_copy_df.apply(
        lambda row: compute_support(row["antecedent_title"] + row["consequent_title"])
        if isinstance(row["antecedent_title"], list) and isinstance(row["consequent_title"], list) else 0,
        axis=1
    )
    
    old_rules_df["antecedent_support"] = old_rules_df["antecedent_support"] + old_copy_df["new_antecedent_support"]
    old_rules_df["consequent_support"] = old_rules_df["consequent_support"] + old_copy_df["new_consequent_support"]
    old_rules_df["support"] = old_rules_df["support"] + old_copy_df["new_support"]
    return old_rules_df

def detect_and_add_new_rules(new_txn_df, old_rules_df, posts_challenges_df, confidence_threshold):
    print("Detecting and adding new rules...")
    frequent_itemsets = apriori(new_txn_df, min_support=0.05, use_colnames=True)
    
    new_rules = association_rules(frequent_itemsets, len(new_txn_df), metric="confidence", min_threshold=confidence_threshold)
    ...
    def is_new_rule(antecedent, consequent):
        for existing_antecedent, existing_consequent in zip(existing_antecedents, existing_consequents):
            if set(antecedent) == existing_antecedent and set(consequent) == existing_consequent:
                return False
        return True
    ...
    new_rules_df.loc[:, "antecedent_id"] = new_rules_df["antecedent_title"].apply(
        lambda x: [posts_challenges_df.loc[posts_challenges_df["challenge_title"] == title, "challenge_id"].iloc[0] for title in x]
    )
...
def recompute_association_rules(merged_rules_df, confidence_threshold):
    ...
    merged_rules_df["confidence"] = merged_rules_df["support"] / merged_rules_df["antecedent_support"]
    merged_rules_df["lift"] = merged_rules_df["confidence"] / merged_rules_df["consequent_support"]
...
async def compute_related_hashtags():
    ...
    columns_to_convert = ["antecedent_support", "consequent_support", "support", "confidence", "lift"]
    old_rules_df[columns_to_convert] = old_rules_df[columns_to_convert].astype(float, errors="ignore")
    ...
    if old_rules_df.empty:
        ...
        updated_rules = association_rules(frequent_itemsets, len(new_txn_df), metric="confidence", min_threshold=confidence_threshold)
        ...
        new_rules_df = detect_and_add_new_rules(new_txn_df, old_rules_df, df, confidence_threshold)
        
        merged_rules_df = pd.concat([merged_rules_df, new_rules_df], ignore_index=True)
        
        final_rules_df = recompute_association_rules(merged_rules_df, confidence_threshold)
